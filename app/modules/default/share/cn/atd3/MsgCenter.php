...
class MsgCenter
{
    ...
    {
        if ($fetch=Query::select('notification', '`#{notification}`.`id`,`send`,`type`,`state`,`#{notification_data}`.`data` ', ' JOIN `#{notification_data}` ON `#{notification_data}`.`id`=`#{notification}`.`data` WHERE `recv` = :recv AND `type`=:type AND `state` != ' .self::STATE_DELETE, ['recv'=>$recv, 'type'=>$type], [$page, $count])->fetchAll()) {
            $ids=[];
