...
class Upload
{
    ...
    {
        if ($select=Query::select('upload', ['uid', 'name', 'size', 'time', 'type', 'hash', 'state'], ' JOIN `#{upload_data}` ON `#{upload_data}`.`id` = `data` WHERE `#{upload}`.`id`=:id AND `state`=:state ', ['id'=>$id])->fetch()) {
            return $select;
        }
        return false;
    }
