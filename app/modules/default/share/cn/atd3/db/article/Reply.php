...
class Reply
{
    ...
    // 回复评论
    public static function create(int $comment, int $author, string $text,int $state=self::STATE_VERIFY)
    {
    ...
    // 回复回复
    public static function reply(int $id, int $comment, int $author, string $text,int $state=self::STATE_VERIFY)
    {
    ...
    // 删除评论
    public static function list(int $comment, int $page=1, int $count=10)
    {
        return Query::where('article_reply', ['id', 'reply', 'comment', 'author', 'text', 'time', 'ip', 'state'], [ 'comment'=>$comment ], [], [$page, $count])->fetchAll();
    }
    
    public static function listAfter(int $id,int $count=10){
